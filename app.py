...
with st.sidebar:
    st.header("🎯 Target Levels")
    st.info("""
    **Reference / Treatment Thresholds:**
    
    * **Potassium ($K^+$):** $\ge$ 3.5 mmol/L [cite: 165]
    * **Magnesium ($Mg$):** $\ge$ 0.7 mmol/L [cite: 151]
    * **Phosphate ($PO_4$):** $\ge$ 0.7 mmol/L [cite: 123]
    * **Calcium ($Ca$):** Monitor daily [cite: 90]
    """)
    ...
    *Rationale: IV phosphate can lower Ca/Mg/K further.* [cite: 97, 98]
    """)
    
    st.markdown("---")
    st.caption("Based on Taunton & Somerset NHS Trust Guidelines")
...
if analyte == "Potassium (K+)":
    ...
    if val_k > 0 and val_k < 3.5:
        ...
        * **ST-segment depression**.
        ...
        if val_k < 2.5:
            st.error("**Treatment Advice:** 40mmol K+ in 1L 0.9% NaCl IV over min 4 hours. Check every 12h[cite: 183, 184].")
            st.warning("NB: Continuous ECG monitoring essential for rates >20mmol/hr[cite: 185].")
        else:
            st.write("**Treatment Advice:** 2 tablets Sando-K TDS/QDS or IV 40mmol K+ over 8 hours[cite: 168, 175].")
...
# MAGNESIUM
    ...
    if val_mg > 0 and val_mg < 0.5:
        st.error("**Treatment Advice:** Give 20mmol Magnesium Sulphate IV over 12 hours. Check serum every 12h[cite: 155, 156].")
    elif 0.5 <= val_mg < 0.7:
        st.warning("**Treatment Advice:** 5ml Magnesium Hydroxide TDS orally until >0.7. Check every 24h[cite: 153, 154].")

# PHOSPHATE
elif analyte == "Phosphate (PO4)":
    val_p = st.number_input("Serum PO4 (mmol/L)", min_value=0.0, step=0.1)
    if val_p > 0 and val_p < 0.3:
        st.error("**Treatment Advice:** Give IV Sodium Glycerophosphate 20mmol over 8-12 hours. Check serum every 12h[cite: 137, 143].")
    elif val_p < 0.5:
        st.warning("**Treatment Advice:** If oral route suitable: 2 tablets Phosphate-Sandoz OD. Otherwise: IV replacement[cite: 129, 131].")
    elif val_p < 0.7:
        st.info("**Treatment Advice:** 1 tablet Phosphate-Sandoz OD. Check serum every 24h[cite: 123, 134].")

# PARENTERAL NUTRITION & CLINICAL ADVICE
st.divider()
st.subheader("Clinical Monitoring Notes")
st.markdown("""
* **Parenteral Nutrition (PN):** Monitor blood glucose every 6 hours[cite: 86].
...
* **Renal Impairment:** Beware of normal K+/PO4 levels in dehydrated patients with renal failure[cite: 87].
""")
